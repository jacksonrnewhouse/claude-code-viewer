...
describe("Artifact Integration — flat structure", () => {
  describe("create + update produces correct versioned artifact", () => {
    it("returns 1 artifact with 2 versions, correct metadata", async () => {
      ...
      const files: Record<string, string> = {
        [`${artifactsDirPath}/manifest.jsonl`]: manifestContent,
        [`${artifactsDirPath}/art-lifecycle-1/v1/index.html`]:
          "<html>v1</html>",
        [`${artifactsDirPath}/art-lifecycle-1/v2/index.html`]:
          "<html>v2</html>",
      };

      const dirs = [
        artifactsDirPath,
        `${artifactsDirPath}/art-lifecycle-1`,
        `${artifactsDirPath}/art-lifecycle-1/v1`,
        `${artifactsDirPath}/art-lifecycle-1/v2`,
      ];
  ...
  describe("multiple artifacts across sessions", () => {
  ...
  describe("getArtifactFile returns file content", () => {
    it("returns the correct bytes for a versioned artifact file", async () => {
      ...
      const files: Record<string, string> = {
        [`${artifactsDirPath}/art-file-1/v1/index.html`]: htmlContent,
        [`${artifactsDirPath}/manifest.jsonl`]: JSON.stringify({
          id: "art-file-1",
          action: "create",
          title: "File Test",
          type: "review",
      ...
      const dirs = [
        artifactsDirPath,
        `${artifactsDirPath}/art-file-1`,
        `${artifactsDirPath}/art-file-1/v1`,
      ];

      const program = Effect.gen(function* () {
        const repo = yield* ArtifactRepository;
        return yield* repo.getArtifactFile("art-file-1", 1, "index.html");
      });
  ...
  describe("missing manifest returns empty", () => {
    it("getAllArtifacts returns [] when no manifest.jsonl exists", async () => {
      const dirs = [artifactsDirPath];
  ...
  describe("getArtifactFile fails for non-existent file", () => {
